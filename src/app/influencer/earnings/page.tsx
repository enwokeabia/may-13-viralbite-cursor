...
export default function EarningsPage() {
  ...
  return (
    <main className="min-h-screen flex flex-col items-center bg-gray-50">
      <div className="w-full max-w-3xl mx-auto p-4 md:p-8 pt-12 pb-16">
        <h1 className="text-2xl md:text-3xl font-bold text-black mb-2">Earnings & Submissions</h1>
        <p className="text-gray-600 mb-6">Track your earnings, stats, and campaign submissions in one place.</p>
        <InfluencerSummaryCards compact />
        <h2 className="text-xl font-semibold text-black mt-8 mb-2">Your Submissions</h2>
          ...
          {submissions.map((sub) => (
            <li key={sub.id} className="border border-gray-200 rounded-xl p-4 bg-white shadow-sm flex flex-col gap-1 text-left">
              ...
              {sub.earnings !== undefined && (
                <div className="text-xs text-green-600 font-semibold">Earnings: ${sub.earnings.toFixed(2)}</div>
              )}
            </li>
          ))}
        </ul>
